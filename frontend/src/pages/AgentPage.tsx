...
const QA_PAIRS: [RegExp, string][] = [
  ...
  [/(what\s+is|what's|show\s+me|give\s+me)\s+(the\s+)?(score|metric|result|value|number)s?\b|score[s]?\s+(in|of)\s+kcal|current\s+score|all\s+(the\s+)?score/i,
    "**Available scores after a design run**\n\n- **ΔG binding** (kcal/mol): modelled free energy; < −6 = lab-worthy\n- **Kd** (nM): dissociation constant from ΔG at 310 K\n- **Stability** (%): secondary structure propensity\n- **Solubility** (%): GRAVY-based estimate\n- **Lab Viability** (0–100): composite Triple-Gate score; ≥ 70 = proceed to synthesis\n\nRun **design a peptide** to start a cycle and populate these scores."],
